...
def SaveShelveCache(name, value):
    ...
    shelveCache[name] = value
    shelveCache.close()

def GetShelveCache(name):    
    try:
        shelveCache = shelve.open('cache')
        return shelveCache['atp']
    except:
        return None
...
def SaveBytesToHtmlFile(title , html, path=None):
    if path != None: 
...
#    for tag in div.findChildren():  tag['class'] = ''
    ...
    html = '''
<thml>
    <body bgcolor="LightGrey" style="font-family:Georgia,Consolas,Arial;">
        <a href="{0}">
            <h3>{1}</h3>
        </a>
        {2}
